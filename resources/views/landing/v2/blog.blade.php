                ...
                <div class="col-sm-12">
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb justify-content-sm-start">
                            ...
                            <li class="breadcrumb-item @if (app('request')->has('id_kabkota')) @else active @endif " aria-current="page">

                                @if (app('request')->has('id_kabkota'))
                                    <a href="{{ config('isec.base_url') }}/blog?category={{ app('request')->input('category') }}">Berita
                                        {{ app('request')->input('category') ? Str::ucfirst(app('request')->input('category')) : 'Semua Berita' }}
                                    </a>
                                @else
                                    Berita {{ app('request')->input('category') ? Str::ucfirst(app('request')->input('category')) : (app('request')->input('author') ? 'oleh ' . ucwords(strtolower(app('request')->input('author'))) : 'Semua Berita') }}
                                @endif
                            </li>
                    ...
                    <div class="page-title">
                        ...
                        @if (app('request')->has('id_kabkota'))
                            <h1 class="greenext" style="border-bottom: 2px solid #dae0e5 !important;">{{ ucwords(strtolower($kabkotaname)) }}
                            </h1>
                        @else
                            <h1 class="greenext" style="border-bottom: 2px solid #dae0e5 !important;">Berita {{ app('request')->input('category') ? Str::ucfirst(app('request')->input('category')) : (app('request')->input('author') ? 'oleh ' . ucwords(strtolower(app('request')->input('author'))) : 'Semua Berita') }}
                            </h1>
                        @endif
                    </div>
                </div>
                                            ...
                                            <div class="row post_footer">
                                                <div class="col-md-4">
                                                    <a href="{{ config('isec.base_url') }}/post/{{ $post->slug }}">
                                                        <img src="{{ $post->rectangle_cover_image }}" alt="letest_post1" class="img-link">
                                                    </a>
                                                </div>
                                                <div class="post_content col-md-8">
                                                    <div class="entry-meta meta-0 font-small mb-10 pb-2">
                                                        @if ($post->category->slug == 'daerah')
                                                            <a href="{{ config('isec.base_url') }}/blog?category={{ app('request')->input('category') }}&id_kabkota={{ $post->id_kabkota }}">
                                                                <span class="badge badge-primary">{{ ucwords(strtolower($post->kabkota->name)) }}</span>
                                                            </a>
                                                        @else
                                                            <a href="{{ config('isec.base_url') }}/blog?category={{ app('request')->input('category') }}">
                                                    ...
                                                    <h5 class="pb-2"><a href="{{ config('isec.base_url') }}/post/{{ $post->slug }}">{{ \Illuminate\Support\Str::limit($post->title, 100, $end = '...') }}</a>
                                                    </h5>
